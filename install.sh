#!/usr/bin/env bash
#
# install.sh - install (or uninstall) the uxauditor command across every AI
# coding tool found on this machine. One source of truth (engine/uxauditor.md)
# rendered into each tool's native skill or slash-command format.
#
# Usage:
#   ./install.sh                    install uxauditor into all detected tools
#   ./install.sh uninstall          remove uxauditor from all detected tools
...
if [ "$MODE" = "install" ]; then
  echo ""
  echo "Invoke it:"
  echo "  Claude Code / Antigravity / pi:  ask \"audit my UX\" (skill auto-triggers), or /$NAME"
